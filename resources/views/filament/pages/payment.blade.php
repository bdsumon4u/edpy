...
<div {{ $attributes->class(['fi-simple-page']) }}>
    <section x-data="{
        ...
        copyToClipboard() {
            navigator.clipboard.writeText(this.method?.meta?.number);
        ...
        <div class="flex items-center justify-between p-2 border rounded-md bg-gray-50">
            <div>
                <svg @click="tab='Mobile Banking'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <section x-cloak>
            ...
            <div x-show="tab=='{{$type}}'" class="grid grid-cols-3 gap-4">
                @forelse($methods->get($type, []) as $method)
                <div @click="setMethod('{{$method}}')" class="flex items-center justify-center">
            ...
            <div x-show="method">
                <div class="p-4 bg-pink-100 rounded-lg">
                    ...
                    <div class="flex items-center justify-between py-1 border-b-2">
                        <span class="text-xl font-semibold text-red-500" x-text="method?.meta?.number"></span>
